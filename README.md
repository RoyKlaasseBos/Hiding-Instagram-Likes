...
2. Open the terminal (Mac) or cmd (Windows), `cd` into above main directory (Hiding-Instagram-Likes), and type `conda create -n instagram --file requirements.txt python=3.7`. This creates a virtual environment in which all packages are installed that are necessary to run the Jupyter notebooks.
